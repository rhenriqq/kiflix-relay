...
const server=http.createServer(async(req,res)=>{if(req.url==='/health'&&req.method==='GET'){res.writeHead(200,{'Content-Type':'application/json'});res.end('{"ok":true}');return}if(req.url!=='/fetch'||req.method!=='POST'||!authenticated(req)){res.writeHead(401);res.end();return}let outgoing;try{let body='';for await(const chunk of req){body+=chunk;if(body.length>16000)throw Error('Corpo excessivo')}const data=JSON.parse(body);let url=new URL(data.url);const range=data.range;if(range&&!/^bytes=\d*-\d*$/.test(range))throw Error('Range inválido');
 for(let i=0;i<8;i++){const ip=await check(url);const upstream=await new Promise((resolve,reject)=>{outgoing=(url.protocol==='https:'?https:http).request(url,{method:'GET',headers:{'User-Agent':'VLC/3.0.20 LibVLC/3.0.20','Accept':'*/*','Accept-Encoding':'identity',...(range?{Range:range}:{})},servername:url.hostname,lookup:(_h,opts,cb)=>opts.all?cb(null,[{address:ip,family:4}]):cb(null,ip,4)},resolve);outgoing.on('error',reject);outgoing.setTimeout(25000,()=>outgoing.destroy(Error('Tempo excedido')));outgoing.end()});if([301,302,303,307,308].includes(upstream.statusCode)){const next=upstream.headers.location;upstream.destroy();if(!next)throw Error('Redirecionamento inválido');url=new URL(next,url);continue}
 const headers={'Cache-Control':'no-store','X-Kiflix-Final-URL':Buffer.from(url.href).toString('base64')};for(const name of ['content-type','content-length','content-range','accept-ranges'])if(upstream.headers[name])headers[name]=upstream.headers[name];res.writeHead(upstream.statusCode,headers);res.on('close',()=>upstream.destroy());upstream.on('error',()=>res.destroy());upstream.pipe(res);return}throw Error('Redirecionamentos excessivos');
 }catch{outgoing?.destroy();if(!res.headersSent)res.writeHead(502,{'Content-Type':'application/json'});res.end('{"error":"Não foi possível acessar a origem de mídia."}')}});
