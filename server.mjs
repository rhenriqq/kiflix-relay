...
const cors={'Access-Control-Allow-Origin':'*','Access-Control-Allow-Methods':'GET,POST,OPTIONS','Access-Control-Allow-Headers':'Authorization,Content-Type,Range','Access-Control-Expose-Headers':'Content-Length,Content-Range,Accept-Ranges,X-Kiflix-Final-URL'};const server=http.createServer(async(req,res)=>{if(req.method==='OPTIONS'){res.writeHead(204,cors);res.end();return}if(req.url==='/health'&&req.method==='GET'){res.writeHead(200,{...cors,'Content-Type':'application/json'});res.end('{"ok":true}');return}if(req.url!=='/fetch'||req.method!=='POST'||!authenticated(req)){res.writeHead(401);res.end();return}let outgoing;try{let body='';for await(const chunk of req){body+=chunk;if(body.length>16000)throw Error('Corpo excessivo')}const data=JSON.parse(body);let url=new URL(data.url);const range=data.range;if(range&&!/^bytes=\d*-\d*$/.test(range))throw Error('Range inválido');
 for(let i=0;i<8;i++){const ip=await check(url);const publicOrigin=publicIP(url.hostname);const originHost=(process.env.ORIGIN_HOST||'').trim();const requestHost=publicOrigin&&originHost?originHost:url.hostname;const upstream=await new Promise((resolve,reject)=>{outgoing=(url.protocol==='https:'?https:http).request(url,{method:'GET',headers:{'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/152 Safari/537.36','Accept':'*/*','Accept-Encoding':'identity','Accept-Language':'pt-BR,pt;q=0.9,en-US;q=0.8','Origin':`${url.protocol}//${requestHost}`,'Referer':`${url.protocol}//${requestHost}/`,...(range?{Range:range}:{}),...(publicOrigin&&originHost?{Host:originHost}:{})},servername:requestHost,lookup:(_h,opts,cb)=>opts.all?cb(null,[{address:ip,family:4}]):cb(null,ip,4)},resolve);outgoing.on('error',reject);outgoing.setTimeout(25000,()=>outgoing.destroy(Error('Tempo excedido')));outgoing.end()});if([301,302,303,307,308].includes(upstream.statusCode)){const next=upstream.headers.location;upstream.destroy();if(!next)throw Error('Redirecionamento inválido');url=new URL(next,url);continue}
